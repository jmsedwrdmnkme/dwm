...
static Key keys[] = {
	...
	{ ShiftMask,		        XF86XK_AudioRaiseVolume,   spawn,          SHCMD("wpctl set-volume -l 1.0 @DEFAULT_AUDIO_SOURCE@ 1%+; kill -45 $(pidof dwmblocks) && vol=$(wpctl get-volume @DEFAULT_AUDIO_SINK@ | tr -d '.' | egrep -o '[0-9]{3}' | sed 's/^0*//') && dunstify -h string:x-dunst-stack-tag:mic \"奔 Microphone volume\" \"$vol%\"") }, // Raise microphone input
	{ ShiftMask,		        XF86XK_AudioLowerVolume,   spawn,          SHCMD("wpctl set-volume @DEFAULT_AUDIO_SOURCE@ 1%-; kill -45 $(pidof dwmblocks) && vol=(wpctl get-volume @DEFAULT_AUDIO_SINK@ | tr -d '.' | egrep -o '[0-9]{3}' | sed 's/^0*//') && dunstify -h string:x-dunst-stack-tag:mic \"奔 Microphone volume\" \"$vol%\"") }, // Lower microphone input
