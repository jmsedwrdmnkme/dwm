...
static Key keys[] = {
	...
	{ 0,			        XF86XK_AudioRaiseVolume,   spawn,          SHCMD("wpctl set-volume @DEFAULT_AUDIO_SINK@ 1%+; kill -39 $(pidof dwmblocks) && vol=\"$(wpctl get-volume @DEFAULT_AUDIO_SINK@ | tr -d '.' | egrep -o '[0-9]{3}' | sed 's/^0*//')\" && dunstify -h string:x-dunst-stack-tag:volume \"奔 Speaker volume\" \"$vol%\"") }, // Raise volume
